...
dir=$(mktemp -d)
tr '\n' '\0' < "$paths" | xargs -0 -P "${CRX_JOBS:-8}" -n 200 sh -c 'exec "$0" "$@" > "'"$dir"'/$$.log" 2>&1 || true' "$bin" $mflag -o "$oracle" "$@"
cat "$dir"/*.log > "$log"; rm -rf "$dir"
grep -v -e '^crxcheck' -e '^headers-ok' -e '^exact' "$log" | head -20
awk '/^crxcheck/ { ok += $(NF-15); n += $(NF-12); u += $(NF-11); f += $(NF-9); no += $(NF-7) } END { printf "TOTAL: %d ok of %d (%d unsupported, %d FAIL, %d without oracle)\n", ok, n, u, f, no; exit (f == 0 && u == 0 && n > 0) ? 0 : 1 }' "$log"
