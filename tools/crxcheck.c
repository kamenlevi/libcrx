...
{
    const char *oracle = NULL; int verbose = 0; unsigned threads = 1; int a = 1;
    ...
    if (!oracle || a >= argc) { fprintf(stderr, "usage: crxcheck [-v] [-t threads] -o oracle.tsv <file.cr3>...\n"); return 2; }
    ...
    for (; a < argc; a++) {
        ...
        row_t key; sha256_hex(bytes, len, key.sha);
    ...
    printf("crxcheck: %zu exact of %zu (%zu unsupported, %zu FAIL, %zu without oracle) in %.1f s, libcrx %s\n",
           exact, judged, unsupported, fail, nooracle, dt, crx_version());
