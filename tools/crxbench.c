...
int main(int argc, char **argv)
{
    ...
    unsigned level = 0, threads = 1; int reps = 5, verbose = 0, use_min = 0, a = 1;
    for (; a < argc && argv[a][0] == '-'; a++) {
        ...
        else if (!strcmp(argv[a], "-m")) use_min = 1;
    ...
    for (int i = 0; i < nfiles; i++) {
        ...
        med[done] = use_min ? times[0] : times[reps / 2];
    ...
    printf("crxbench: level %u, %u thread%s, %d files (%d skipped): %s %.2f ms, p90 %.2f ms, worst %.2f ms, %.0f Mpix/s, libcrx %s\n",
           level, threads, threads == 1 ? "" : "s", done, skipped, use_min ? "median-of-min" : "median",
